...
const EnhancedSlackAlertSystem = () => {
    ...
    // Enhanced mock alerts (same as before)
    const alerts = [
        {
    ...
    // Metric definitions (same as before)
    const metricIcons = {
    ...
    // GPT response generation (same as before)
    ...
    const gptResponse = generateGPTResponse(alert); // Ensure alert is defined before calling
    ...
    const handleFeedback = useCallback((feedbackType) => {
        console.log(`Feedback submitted for alert index ${currentAlertIndex}: ${feedbackType}`);
        setFeedbackGiven(prev => ({
            ...prev,
            [currentAlertIndex]: feedbackType
        }));
    }, [currentAlertIndex]);

    // Check if feedback was given for the current alert
